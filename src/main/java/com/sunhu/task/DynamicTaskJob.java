...
public class DynamicTaskJob {
    ...
    private static final Logger logger = LogManager.getLogger(DynamicTaskJob.class);
     ...
     * 动态设置定时任务方法
     *
     * 此方法是真正的动态实现启停和时间周期的关键，你可以针对自己的业务来调用，你对库中的动态数据修改后来调用此方法，
     * 每个Cron对象必须要包含，执行周期（cron.getCron()），启停状态（cron.getCronStatus()），执行的类（cron.getCronClass()）
     */
